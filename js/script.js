document.addEventListener("DOMContentLoaded", () => {
    ...
    const PROJECT_STORAGE_KEY = "devboard_projects_v1";
    ...
    function addTask(text, priority, projectId) {
        ...
        addActivity(
            ...
            "New task added",
            relatedProject
                ? `"${cleanText}" was added to "${relatedProject.name}".`
                : `"${cleanText}" was added as an individual task.`
        );
    }
    ...
    function renderProjectSelect() {
        ...
        individualOption.textContent = "No Project / Individual Task";

        taskProject.appendChild(individualOption);
        ...
        if (
            currentValue &&
            projects.some((project) => project.id === currentValue)
        ) {
    ...
    function renderProjects() {
        ...
        if (projects.length === 0) {
            ...
            emptyState.className = "empty-project-state";
            emptyState.textContent =
        ...
        projects.forEach((project, index) => {
            ...
            projectTop.appendChild(statusElement);
            ...
            taskSummary.textContent =
                `${completedProjectTasks} of ${projectTasks.length} tasks completed`;

            const progressText = document.createElement("span");

            progressText.textContent = `${progress}%`;

            projectMeta.appendChild(taskSummary);
            projectMeta.appendChild(progressText);
            ...
            projectProgress.appendChild(progressFill);

            projectInfo.appendChild(projectTop);
            projectInfo.appendChild(description);
            projectInfo.appendChild(projectMeta);
            projectInfo.appendChild(projectProgress);
    ...
    if (taskForm) {
        taskForm.addEventListener("submit", (event) => {
            ...
            addTask(
                ...
                taskProject ? taskProject.value || null : null
            );
    ...
    function createProfileUI() {
        ...
        profileButton.innerHTML = `
            ...
            <span class="profile-info"><span class="profile-name">Profile</span><span class="profile-company"></span></span>
        `;
    ...
    function updateProfileDisplay() {
        ...
        const company =
            profileButton.querySelector(".profile-company");
...
.profile-info { display:flex; flex-direction:column; align-items:flex-start; justify-content:center; line-height:1.15; } .profile-name { font-weight:600; font-size:13px; } .profile-company { font-size:10px; opacity:.55; margin-top:3px; max-width:150px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; } .profile-avatar,
